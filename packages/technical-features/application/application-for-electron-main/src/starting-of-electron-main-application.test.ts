...
describe("starting-of-electron-main-application", () => {
  ...
  beforeEach(() => {
    ...
    di.override(startApplicationInjectionToken, (di) => {
      const runSync = runManySyncFor(di);
      const runAsync = runManyFor(di);
      const whenAppIsReady = di.inject(whenAppIsReadyInjectable);

      const beforeAnything = runSync(timeSlots.beforeAnythingInjectionToken);
      const beforeElectronIsReady = runSync(timeSlots.beforeElectronIsReadyInjectionToken);
      const beforeApplicationIsLoading = runAsync(beforeApplicationIsLoadingInjectionToken);
