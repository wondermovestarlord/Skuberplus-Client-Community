...
export class LensProxy {
  ...
  constructor(private readonly dependencies: Dependencies) {
    ...
    this.proxyServer.on("upgrade", (req: ServerIncomingMessage, socket: net.Socket, head: Buffer) => {
      // 🔍 Phase 2: 디버그 로깅 - Upgrade 요청 수신
      this.dependencies.logger.info(`[LENS-PROXY] WebSocket upgrade 요청 수신:`, {
        url: req.url,
        method: req.method,
        headers: {
      ...
      // 🔍 Phase 2: 디버그 로깅 - getClusterForRequest 결과
      this.dependencies.logger.info(`[LENS-PROXY] getClusterForRequest 결과:`, {
        clusterFound: !!cluster,
        clusterId: cluster?.id || "N/A",
      ...
      if (!cluster) {
        ...
        socket.write("Content-Length: 18\r\n");
        socket.write("Connection: close\r\n");
        socket.write("\r\n");
        socket.write("Cluster not found");
        socket.end();
      } else {
        ...
        // 🔍 Phase 2: 디버그 로깅 - Handler 선택
        this.dependencies.logger.info(`[LENS-PROXY] Upgrade handler 선택:`, {
          isInternal,
          handlerType: isInternal ? "shellApiRequest" : "kubeApiUpgradeRequest",
