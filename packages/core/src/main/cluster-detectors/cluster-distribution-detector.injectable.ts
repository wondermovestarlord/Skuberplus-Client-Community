...
// 🎯 Naver Cloud NKS (Naver Kubernetes Service) 감지
// Server URL 패턴: *.vnks.ntruss.com (예: 360582b6-xxx.kr.vnks.ntruss.com)
const isNKS = (apiUrl: URL) => apiUrl.hostname.endsWith("vnks.ntruss.com");
  ...
  instantiate: (di) => {
    ...
    return {
      ...
      detect: async (cluster) => {
        ...
        // 🎯 Naver Cloud NKS 감지 (vnks.ntruss.com 도메인 기반)
        if (isNKS(apiUrl)) {
          return { value: "nks", accuracy: 90 };
        }
