...
// 🎯 macOS fd 제한(256) 대응: Kube Auth Proxy 동시 실행 제한
const kubeAuthProxyLimit = pLimit(5);
...
import skuberPlusK8sProxyPathInjectable from "./skuberplus-k8s-proxy-path.injectable";
...
const createKubeAuthProxyInjectable = getInjectable({
  ...
  instantiate: (di, cluster): CreateKubeAuthProxy => {
    const skuberPlusK8sProxyPath = di.inject(skuberPlusK8sProxyPathInjectable);
    ...
    return (env) => {
      ...
      const run = async (): Promise<void> => {
        ...
        // 🎯 pLimit 적용: 동시 Kube Auth Proxy 프로세스 제한
        proxyProcess = await kubeAuthProxyLimit(() =>
          Promise.resolve(
            spawn(skuberPlusK8sProxyPath, [], {
              env: {
                ...env,
