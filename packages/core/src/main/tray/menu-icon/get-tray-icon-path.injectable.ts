...
const getTrayIconPathInjectable = getInjectable({
  ...
  instantiate: (di) => {
    ...
    const baseIconDirectory = joinPaths(staticFilesDirectory, "build/tray");

    const fileSuffix = isMac ? "Template.png" : ".png";

    return (name: string) => joinPaths(baseIconDirectory, `trayIcon${upperCamelCase(name)}${fileSuffix}`);
  },
});
