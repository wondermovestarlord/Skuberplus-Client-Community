...
describe("ensure-hashed-directory-for-extension", () => {
  ...
  describe("given extension directory was saved based on extension's package.json path", () => {
    beforeEach(() => {
      runInAction(() => {
        registeredExtensions.set(
          "/some-directory-for-user-data/node_modules/some-extension-name/package.json",
          "some-directory",
        );
      });
    ...
    it("ensure dir is called with some directory", async () => {
      await ensureHashedDirectoryForExtension("some-extension-name");
    ...
    it("old key is removed", async () => {
      ...
      expect(
        registeredExtensions.has("/some-directory-for-user-data/node_modules/some-extension-name/package.json"),
      ).toEqual(false);
    });
  });
});
