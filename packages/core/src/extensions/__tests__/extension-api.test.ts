...
describe("Extensions API", () => {
  ...
  describe("Renderer API", () => {
    ...
    describe("Component namespace", () => {
      ...
      // notificationsStore는 Sonner 마이그레이션으로 제거됨
      it("should not export removed notificationsStore", () => {
        expect(extensions.Renderer.Component).not.toHaveProperty("notificationsStore");
      });
