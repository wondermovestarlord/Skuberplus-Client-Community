...
@observer
class NonInjectedKubeObjectMenu<Kube extends KubeObject> extends Component<KubeObjectMenuProps<Kube> & Dependencies> {
  ...
  /** Shortcut hint mappings for known menu action IDs */
  private static kbdHints: Record<string, string> = {
    "edit-kube-object": "e",
    "delete-kube-object-delete": "\u2303D",
  };
  ...
  private renderContextMenuItems = (object: KubeObject) => {
    ...
    return [...this.menuItems]
      ...
      .map((item, index) => {
        const kbdHint = item.id ? NonInjectedKubeObjectMenu.kbdHints[item.id] : undefined;
        ...
        return (
          ...
          >
            ...
            {kbdHint && <Kbd className="ml-auto">{kbdHint}</Kbd>}
          </MenuItem>
        );
      });
  };
