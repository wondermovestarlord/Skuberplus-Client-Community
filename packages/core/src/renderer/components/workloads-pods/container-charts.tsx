...
/**
 * 🎯 목적: Pod Container 메트릭 차트 컴포넌트 (shadcn Chart 기반)
 *
 * @remarks
 * - Chart.js → shadcn Chart (recharts) 전환
 * - CPU/Memory 메트릭을 AreaChart로 시각화
 * - Usage, Requests, Limits 3가지 데이터셋 표시
 *
 * 📝 주의사항:
 * - Prometheus 메트릭 데이터 형식: [timestamp, value][]
 * - recharts 형식으로 변환 필요: { time: number, usage: number, requests: number, limits: number }[]
 *
 * 🔄 변경이력:
 * - 2025-10-30: Chart.js → shadcn Chart (recharts) 전환
 */
 ...
 * @param requests - Requests 메트릭 데이터 [timestamp, value][]
 * @param limits - Limits 메트릭 데이터 [timestamp, value][]
 * @returns recharts 형식의 데이터 배열
 */
function convertToChartData(
  usage: Array<[number, string]>,
  requests: Array<[number, string]>,
  limits: Array<[number, string]>,
): Array<{ time: number; usage: number; requests: number; limits: number }> {
  // 🎯 모든 타임스탬프 수집 (중복 제거)
  // 🛡️ null/undefined 방어 처리 - Prometheus 미설치 클러스터에서 크래시 방지
  const timestamps = new Set<number>();
  (usage ?? []).forEach(([time]) => timestamps.add(time));
  (requests ?? []).forEach(([time]) => timestamps.add(time));
  (limits ?? []).forEach(([time]) => timestamps.add(time));

  // 🎯 타임스탬프별 데이터 맵 생성
  const usageMap = new Map((usage ?? []).map(([time, value]) => [time, parseFloat(value)]));
  const requestsMap = new Map((requests ?? []).map(([time, value]) => [time, parseFloat(value)]));
  const limitsMap = new Map((limits ?? []).map(([time, value]) => [time, parseFloat(value)]));
...
export const ContainerCharts = observer(({}: ContainerChartsProps) => {
  const { metrics, tab, object } = useContext(ResourceMetricsContext) ?? {};

  // 🎯 메트릭 데이터 추출 (hooks 순서 유지를 위해 early return 전에 처리)
  const { cpuUsage, cpuRequests, cpuLimits, memoryUsage, memoryRequests, memoryLimits } = mapValues(
    metrics || {},
  ...
  const chartData = useMemo(
    () =>
      isCPU
        ? convertToChartData(cpuUsage, cpuRequests, cpuLimits)
        : convertToChartData(memoryUsage, memoryRequests, memoryLimits),
    [isCPU, cpuUsage, cpuRequests, cpuLimits, memoryUsage, memoryRequests, memoryLimits],
  );
  ...
  // 📝 주의사항: Storybook 템플릿 패턴 적용 (var(--chart-1), var(--chart-2), var(--chart-3))
  // 🔄 변경이력: 2025-11-04 - 하드코딩 색상 → 디자인 토큰 마이그레이션 (테마 대응 개선)
  ...
  const chartConfig = {
    ...
    limits: {
      label: `${labelPrefix} Limits`,
      color: isCPU ? "var(--chart-3)" : "var(--chart-4)",
    },
    ...
    <div className="h-[240px] w-full">
      <ChartContainer config={chartConfig} className="h-full w-full" style={{ width: "100%", height: "100%" }}>
        ...
        >
            ...
            content={
              <ChartTooltipContent
                ...
                formatter={(value, name, item) => {
                  ...
                  const formattedValue = isCPU ? formatCPU(Number(value)) : formatMemory(Number(value));
          ...
          />
          {/* Usage Area */}
          {/* 🔄 변경이력: 2026-01-26 - type="step" → "monotone"으로 변경하여 부드러운 곡선 렌더링 */}
          <Area
            dataKey="usage"
            type="monotone"
          ...
          <Area
            dataKey="limits"
            type="monotone"
            fill="var(--color-limits)"
            fillOpacity={0.4}
            stroke="var(--color-limits)"
            isAnimationActive={false}
          />
