  ...
  instantiate: (di, job) => {
    ...
    return asyncComputed({
      ...
      getValueFromObservedPromise: async () => {
        ...
        try {
          let metricsData: JobPodMetricData;
          ...
          if (isMetricsServerSelected) {
            ...
            if (childPods.length > 0) {
              const metrics = await retryMetricsRequest(() => podMetricsApi.list({ namespace: job.getNs() }), {
                label: "job-metrics:list",
              });
              // 🔄 변경이력: 2026-01-26 - Race Condition 해결
              //   loadKubeMetrics 반환값을 직접 사용하여 MobX observable 업데이트 타이밍 문제 우회
              const loadedMetrics = await retryMetricsRequest(() => podStore.loadKubeMetrics(job.getNs()), {
                label: "job-metrics:load",
              });
              ...
              if (matchedPods === 0) {
                const lastValues = metricsCache.getLastValues(job);
                if (lastValues) {
              ...
              console.warn(`⚠️ [JOB-METRICS] No child pods found for job ${job.getName()}`);
              metricsData = {
            ...
            metricsData = await requestPodMetricsForJobs([job], job.getNs());
          }
          ...
          if (cpuValues && cpuValues.length === 1 && memoryValues && memoryValues.length === 1) {
            ...
            metricsCache.addMetricDataPoint(job, cpuValue, memoryValue);

            let cpuChartData = metricsCache.getCpuChartData(job);
            let memoryChartData = metricsCache.getMemoryChartData(job);
            ...
            const cachedMetricData: JobPodMetricData = {
              cpuUsage: {
                status: "success",
                data: {
                  resultType: "matrix",
                  result: [
                    {
                      metric: metricsData.cpuUsage.data.result[0]?.metric || { job: job.getName() },
              ...
              memoryUsage: {
                ...
                data: {
                  ...
                  result: [
                    {
                      metric: metricsData.memoryUsage.data.result[0]?.metric || { job: job.getName() },
          ...
          console.warn(`❌ [JOB-METRICS] Failed to load metrics for ${job.getName()}:`, error);
