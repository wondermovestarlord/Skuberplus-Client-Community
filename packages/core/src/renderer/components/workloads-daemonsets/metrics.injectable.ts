...
import type { DaemonSet, PodMetrics } from "@skuberplus/kube-object";
...
/**
 * 🎯 목적: DaemonSet 상세 화면에서 메트릭 데이터를 Prometheus 호환 형식으로 제공
 *
 * ✅ 주요 기능:
 * - Metrics Server 선택 시: podMetricsApi + podStore 사용 (Pod 메트릭과 동일한 방식)
 *   - Child Pod들의 실제 메트릭 값을 합산하여 DaemonSet 전체 메트릭 제공
 *   - 시계열 데이터 축적하여 그래프 표시
  ...
  instantiate: (di, daemonSet) => {
    ...
    return asyncComputed({
      ...
      getValueFromObservedPromise: async () => {
        ...
        try {
          let metricsData: DaemonSetPodMetricData;
          ...
          if (isMetricsServerSelected) {
            ...
            if (childPods.length > 0) {
              const metrics = await retryMetricsRequest(() => podMetricsApi.list({ namespace: daemonSet.getNs() }), {
                label: "daemonset-metrics:list",
              });
              // 🔄 변경이력: 2026-01-26 - Race Condition 해결
              //   loadKubeMetrics 반환값을 직접 사용하여 MobX observable 업데이트 타이밍 문제 우회
              const loadedMetrics = await retryMetricsRequest(() => podStore.loadKubeMetrics(daemonSet.getNs()), {
                label: "daemonset-metrics:load",
              });
              ...
              if (matchedPods === 0) {
                const lastValues = metricsCache.getLastValues(daemonSet);
                if (lastValues) {
              ...
              metricsData = {
                cpuUsage: {
                  ...
                  data: {
                    ...
                    result: [
                      {
                        metric: { daemonset: daemonSet.getName() },
              ...
              console.warn(`⚠️ [DAEMONSET-METRICS] No child pods found for daemonSet ${daemonSet.getName()}`);
              metricsData = {
            ...
            metricsData = await requestPodMetricsForDaemonSets([daemonSet], daemonSet.getNs());
          }
          ...
          if (cpuValues && cpuValues.length === 1 && memoryValues && memoryValues.length === 1) {
            ...
            metricsCache.addMetricDataPoint(daemonSet, cpuValue, memoryValue);

            let cpuChartData = metricsCache.getCpuChartData(daemonSet);
            let memoryChartData = metricsCache.getMemoryChartData(daemonSet);
            ...
            const cachedMetricData: DaemonSetPodMetricData = {
              cpuUsage: {
                status: "success",
                data: {
                  resultType: "matrix",
                  result: [
                    {
                      metric: metricsData.cpuUsage.data.result[0]?.metric || { daemonset: daemonSet.getName() },
              ...
              memoryUsage: {
                ...
                data: {
                  ...
                  result: [
                    {
                      metric: metricsData.memoryUsage.data.result[0]?.metric || { daemonset: daemonSet.getName() },
          ...
          console.warn(`❌ [DAEMONSET-METRICS] Failed to load metrics for ${daemonSet.getName()}:`, error);
