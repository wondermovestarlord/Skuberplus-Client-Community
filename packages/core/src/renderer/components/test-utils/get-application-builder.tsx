...
type RenderFn = ReturnType<typeof renderFor>;
type RenderOutput = ReturnType<RenderFn>;

type LensWindowWithHelpers = LensWindow & { rendered: RenderOutput; di: DiContainer };
...
export const getApplicationBuilder = (user: UserEvent = userEvent.setup()) => {
  ...
  mainDi.override(directoryForUserDataInjectable, () => "/tmp/daive-user-data");
  mainDi.override(directoryForTempInjectable, () => "/tmp/daive-temp");
  const lensLocalStorageDirectory = "/tmp/daive-lens-local-storage";
  mainDi.override(directoryForLensLocalStorageInjectable, () => lensLocalStorageDirectory);

  const mockAppPaths = pathNames.reduce<AppPaths>((acc, name) => {
    acc[name] = `/mock/${name}`;

    return acc;
  }, {} as AppPaths);

  mainDi.override(appPathsStateInjectable, () => {
    let state: AppPaths | undefined = mockAppPaths;
  ...
  const createElectronWindowFake: CreateElectronWindow = (configuration) => {
    ...
    windowDi.permitSideEffects(storesAndApisCanBeCreatedInjectable);
    windowDi.permitSideEffects(storesAndApisCanBeCreatedInjectionToken);
    windowDi.permitSideEffects(namespaceApiInjectable);
    ...
    runInAction(() => {
      ...
      windowDi.override(storesAndApisCanBeCreatedInjectionToken, () => true);
      windowDi.override(namespaceApiInjectable, () => namespaceApiStub as any);
  ...
  const builder: ApplicationBuilder = {
    ...
    preferences: {
      close: () => {
        ...
        const closeDialog = windowDi.inject(closePreferencesDialogInjectable);
    ...
    setEnvironmentToClusterFrame: () => {
      ...
      builder.beforeWindowStart(({ windowDi }) => {
        ...
        windowDi.override(getClusterByIdInjectable, () => (id: string) => (id === cluster.id ? cluster : undefined));
  ...
  builder.beforeWindowStart(({ windowDi }) => {
    windowDi.override(directoryForLensLocalStorageInjectable, () => lensLocalStorageDirectory);

    windowDi.override(appPathsStateInjectable, () => {
      let state: AppPaths | undefined = mockAppPaths;
...
type ApplicationWindowHelpers = Map<string, { di: DiContainer; getRendered: () => RenderOutput }>;
