  ...
  instantiate: (di): AttemptInstallByInfo => {
    ...
    return async (info) => {
      ...
      try {
        ...
        if (!result.callWasSuccessful) {
          notificationPanelStore.addError(
            "extensions",
            "Registry Error",
            `Failed to get registry information for extension: ${result.error}`,
          );
        ...
        if (!isObject(result.response) || Array.isArray(result.response)) {
          notificationPanelStore.addError(
            "extensions",
            "Registry Error",
            "Failed to get registry information for extension",
          );
        ...
        if (result.response.error || !isObject(result.response.versions)) {
          ...
          notificationPanelStore.addError(
            "extensions",
            "Registry Error",
            `Failed to get registry information for extension: ${message}`,
          );
        ...
        if (error instanceof SyntaxError) {
          ...
          notificationPanelStore.addError(
            "extensions",
            "Registry Error",
            `Failed to get valid registry information for extension. ${error}`,
          );
        }
            ...
            if (potentialVersion) {
              if (!json.versions[potentialVersion]) {
                notificationPanelStore.addError(
                  "extensions",
                  "Registry Error",
                  `Configured registry claims to have tag "${versionOrTagName}". But does not have version information for the reference.`,
                );
      ...
      if (!version) {
        ...
        notificationPanelStore.addError("extensions", "Version Not Found", `No versions found for ${name}`);
      ...
      if (!tarballUrl) {
        notificationPanelStore.addError(
          "extensions",
          "Registry Error",
          "Configured registry has invalid data model. Please verify that it is like NPM's.",
        );
        logger.warn(
      ...
      if (!request.callWasSuccessful) {
        notificationPanelStore.addError(
          "extensions",
          "Download Failed",
          `Failed to download extension: ${request.error}`,
        );
