...
class NonInjectedCreateResource extends Component<CreateResourceProps & Dependencies> {
  @observable error = "";
    ...
    const creatingResources = resources.map(async (resource) => {
      ...
      if (!result.callWasSuccessful) {
        ...
        notificationPanelStore.addCheckedError(
          "operations",
          result.error,
          "Unknown error occurred while creating resources",
          { clusterName },
        );
      ...
      notificationPanelStore.addSuccess("operations", "Resource Created", `${kind} ${name} successfully created.`, {
        actionType: "create",
        resourceKind: kind,
        resourceName: name,
        namespace,
...
export const CreateResource = withInjectables<Dependencies, CreateResourceProps>(observer(NonInjectedCreateResource), {
