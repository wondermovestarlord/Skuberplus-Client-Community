...
describe("<PodDisruptionBudgets />", () => {
  ...
  // TODO: PodDisruptionBudgets 컴포넌트가 shadcn UI로 마이그레이션되면서 인터페이스가 변경되어 테스트 업데이트 필요
  describe.skip("PDB with minAvailable 0", () => {
    const pdb = getPdb({
      minAvailable: 0,
    });
  ...
  describe.skip("PDB with maxUnavailable 0", () => {
    const pdb = getPdb({
      maxUnavailable: 0,
    });
