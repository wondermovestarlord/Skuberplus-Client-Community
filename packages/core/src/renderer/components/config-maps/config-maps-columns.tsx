...
/**
 * 🎯 목적: ConfigMap 테이블 컬럼 정의
 *
 * 컬럼 구성:
 * - Name: ConfigMap 이름
 * - Namespace: 네임스페이스
 * - Keys: ConfigMap에 포함된 키 목록 (배열 → 문자열 join)
 * - Age: 생성 시간
 *
 * 📝 주의사항:
 * - ColumnDef<ConfigMap>[] 타입 사용
 * - accessorFn으로 ConfigMap 객체 메서드 호출
...
import type { ConfigMap } from "@skuberplus/kube-object";
...
export const configMapColumns: ColumnDef<ConfigMap>[] = [
  ...
  // ============================================
  {
    ...
    accessorFn: (configMap) => configMap.getName(),
    cell: ({ row }) => {
  ...
  // ============================================
  {
    ...
    accessorFn: (configMap) => configMap.getNs(),
    cell: ({ row }) => {
      const configMap = row.original;
      return (
        <div className="px-3">
          <NamespaceSelectBadge namespace={configMap.getNs()} />
        </div>
      );
    },
    enableSorting: true,
  ...
  },
  {
    ...
    accessorFn: (configMap) => configMap.getKeys().join(", "),
    cell: ({ row }) => {
      const keys = row.original.getKeys();
      const keysText = keys.join(", ");
      return (
        <div className="px-3 text-sm text-muted-foreground truncate" title={keysText}>
          {keysText || "-"}
        </div>
      );
    },
  ...
  // ============================================
  {
    ...
    accessorFn: (configMap) => configMap.metadata.creationTimestamp,
    cell: ({ row }) => {
