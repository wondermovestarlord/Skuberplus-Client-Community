...
import { configMapApiInjectable, storesAndApisCanBeCreatedInjectionToken } from "@skuberplus/kube-api-specifics";
...
const configMapStoreInjectable = getInjectable({
  ...
  instantiate: (di) => {
    assert(
      ...
      "configMapStore is only available in certain environments",
    );
