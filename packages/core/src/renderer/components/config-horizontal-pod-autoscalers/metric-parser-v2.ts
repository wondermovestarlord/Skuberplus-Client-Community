...
export class HorizontalPodAutoscalerV2MetricParser {
  ...
  public getExternal({
    ...
    return {
      current: current?.current?.value ?? currentAverage,
      target: target?.target?.value ?? targetAverage,
    };
  }
  ...
  public getContainerResource({
    ...
    current: V2ContainerResourceMetricStatus | undefined;
    target: V2ContainerResourceMetricSource;
  }): MetricCurrentTarget {
    return {
      current:
        current?.current?.averageValue ??
