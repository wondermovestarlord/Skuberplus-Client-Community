...
/**
 * 🎯 목적: MutatingWebhookConfiguration 상세 정보를 우측 슬라이드 패널로 표시
 * 📝 주의사항:
 *   - shadcn DetailPanel 컴포넌트 사용
 *   - shadcn Table로 메타데이터 표시 (Created, Name, Webhooks)
 *   - MutatingWebhookConfiguration은 cluster-scoped 리소스 (namespace 없음)
...
import { MutatingWebhookConfiguration } from "@skuberplus/kube-object";
...
/**
 * 🎯 목적: MutatingWebhookConfigurationDetailPanel Props 인터페이스
 */
export interface MutatingWebhookConfigurationDetailPanelProps {
  /**
   * 패널 열림/닫힘 상태
   */
  isOpen: boolean;
  ...
  /**
   * 표시할 MutatingWebhookConfiguration 객체
   */
  config: MutatingWebhookConfiguration | undefined;
...
/**
 * 🎯 목적: MutatingWebhookConfiguration 상세 정보 우측 슬라이드 패널 컴포넌트
 */
const NonInjectedMutatingWebhookConfigurationDetailPanel = observer(
  ({
    isOpen,
    config,
    onClose,
    ...
    openConfirmDialog,
    hostedCluster,
  }: MutatingWebhookConfigurationDetailPanelProps & Dependencies) => {
    const clusterName = hostedCluster?.name.get() ?? "Unknown Cluster";
    // ============================================
    // 🎯 닫힘 애니메이션 동안 마지막 선택 항목 유지
    // ============================================
    const [renderConfig, setRenderConfig] = useState<MutatingWebhookConfiguration | undefined>(config);
    ...
    if (!(item instanceof MutatingWebhookConfiguration)) {
      logger.error(
        "[MutatingWebhookConfigurationDetailPanel]: passed object that is not an instanceof MutatingWebhookConfiguration",
        item,
      );
      return null;
    }
    ...
    /**
     * Delete 액션: MutatingWebhookConfiguration 삭제
     */
    const handleDelete = () => {
      openConfirmDialog({
        ok: async () => {
          try {
            ...
            notificationPanelStore.addError(
              ...
              error instanceof Error
                ? error.message
                : "Unknown error occurred while deleting mutating webhook configuration",
              { clusterName },
            );
          }
        },
        ...
        message: (
          <p>
            Are you sure you want to delete mutating webhook configuration <b>{item.getName()}</b>?
          </p>
        ),
      });
    };
...
/**
 * DI 패턴 적용된 MutatingWebhookConfiguration Detail Panel
 */
export const MutatingWebhookConfigurationDetailPanel = withInjectables<
  Dependencies,
  MutatingWebhookConfigurationDetailPanelProps
>(NonInjectedMutatingWebhookConfigurationDetailPanel, {
  getProps: (di, props) => ({
    ...props,
