...
/**
 * 🎯 목적: Worker 노드들의 클러스터 메트릭 데이터 수집
 *
 * ✅ 주요 기능:
 ...
 * - Worker 노드만 필터링 (Master 노드가 아닌 모든 노드)
 *
 * 🔄 변경이력:
 * - 2025-11-06: Metrics Server 지원 추가
 */
const workerClusterMetricsInjectable = getInjectable({
  id: "worker-cluster-metrics",
  instantiate: (di) => {
    ...
    return asyncComputed<ClusterMetricData | undefined>({
      getValueFromObservedPromise: async () => {
        ...
        // 🎯 Worker 노드만 필터링 (Master가 아닌 모든 노드)
        const workerNodes = nodeStore.items.filter((node) => {
        ...
        const nodeNames = workerNodes.map((node) => node.getName());
        ...
        if (isMetricsServerSelected) {
          ...
          // 🎯 Worker 노드에서 실행 중인 모든 Pod 가져오기
          const workerPods = podStore.items.filter((pod) => nodeNames.includes(pod.spec?.nodeName || ""));

          // 🎯 실제 사용 중인 Pod 수 = 해당 노드에서 실행 중인 Pod 개수
          totalPodUsage = workerPods.length;
          ...
          workerNodes.forEach((node, index) => {
            ...
            // 📝 주의: totalPodUsage는 위에서 workerPods.length로 이미 설정됨
...
export default workerClusterMetricsInjectable;
