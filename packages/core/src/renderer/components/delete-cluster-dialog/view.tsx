...
class NonInjectedDeleteClusterDialog extends Component<Dependencies> {
  async onDelete(state: DeleteClusterDialogState) {
    ...
    runInAction(() => {
      this.props.state.set({
        ...
        config: Object.assign(config, {
          ...
          currentContext: newCurrentContext && showContextSwitch ? newCurrentContext : config.currentContext,
        }),
      });
    });
    ...
    try {
      ...
      /**
       * 🎯 목적: 클러스터 삭제 성공 후 카탈로그로 네비게이션
       * 📝 주의: cluster-view.tsx의 reaction이 MobX 타이밍 문제를 처리하지만,
       *         명시적 네비게이션으로 사용자 경험을 개선
       * 🔄 변경이력: 2026-01-04 - 해결
       */
      ...
      notificationPanelStore.addError(
        ...
        `Cannot remove cluster, failed to process config file. ${error}`,
      );
    } finally {
  ...
  shouldDeleteBeDisabled({ cluster, config, newCurrentContext, showContextSwitch }: DeleteClusterDialogState): boolean {
    const noContextsAvailable =
      config.contexts.filter((context) => context.name !== cluster.contextName.get()).length == 0;
  ...
  renderCurrentContextSwitch(state: DeleteClusterDialogState) {
    ...
    const selectOptions = config.contexts
...
export const DeleteClusterDialog = withInjectables<Dependencies>(observer(NonInjectedDeleteClusterDialog), {
  getProps: (di) => ({
