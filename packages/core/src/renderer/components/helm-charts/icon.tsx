...
/**
 * 🎯 목적: Helm Chart 아이콘 컴포넌트
 * 📝 주의사항:
 *   - base64 SVG와 HTTP URL 모두 동일한 크기로 표시
 *   - placeholder 아이콘은 imageUrl이 없거나 로드 실패 시 표시
 *   - 이미지 로드 실패 감지를 위해 useEffect로 미리 검증
 */
export const HelmChartIcon = ({ imageUrl = "", className }: HelmChartIconProps) => {
  ...
  // 🎯 이미지 로드 검증을 위한 상태 관리
  const [effectiveUrl, setEffectiveUrl] = React.useState(placeholderImageUrl);

  // 🎯 이미지 URL이 변경될 때마다 로드 테스트
  React.useEffect(() => {
    const url = imageUrl?.trim();

    // 빈 URL이면 placeholder 사용
    if (!url) {
      setEffectiveUrl(placeholderImageUrl);
      return;
    }

    // base64 data URL이면 바로 사용
    if (url.startsWith("data:")) {
      setEffectiveUrl(url);
      return;
    }

    // HTTP/HTTPS URL은 로드 테스트
    const img = new Image();
    img.onload = () => setEffectiveUrl(url);
    img.onerror = () => setEffectiveUrl(placeholderImageUrl);
    img.src = url;
  }, [imageUrl, placeholderImageUrl]);

  return (
    <div
      className={cssNames(styles.chartIcon, className)}
