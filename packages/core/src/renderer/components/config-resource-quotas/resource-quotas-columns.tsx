 ...
 *
 * 📝 주의사항:
 * - ColumnDef<ResourceQuota>[] 타입 사용
 * - accessorFn으로 ResourceQuota 객체 메서드 호출
 * - 모든 컬럼 항상 표시 (컬럼 수 적음)
 *
 * 🔄 변경이력:
 ...
 * 🎯 목적: ResourceQuota 테이블 컬럼 정의
 */
export const resourceQuotaColumns: ColumnDef<ResourceQuota>[] = [
  // ============================================
  // 🔲 Checkbox 컬럼 (행 선택)
  // ============================================
  {
  ...
  // ============================================
  {
    ...
    accessorFn: (quota) => quota.getName(),
    cell: ({ row }) => {
  ...
  // ============================================
  {
    ...
    accessorFn: (quota) => quota.getNs(),
    cell: ({ row }) => {
      const quota = row.original;
      return (
        <div className="px-3">
          <NamespaceSelectBadge namespace={quota.getNs()} />
        </div>
      );
    },
    enableSorting: true,
  ...
  // ============================================
  {
    ...
    accessorFn: (quota) => quota.metadata.creationTimestamp,
    cell: ({ row }) => {
