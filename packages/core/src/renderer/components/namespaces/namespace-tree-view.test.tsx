...
jest.mock("@mui/utils", () => {
  const actual = jest.requireActual<typeof import("@mui/utils")>("@mui/utils");
...
describe("<NamespaceTreeView />", () => {
  let di: DiContainer;
  let render: DiRender;
  let reactUseIdSpy: jest.SpiedFunction<typeof React.useId>;
  const muiUseIdMock = MuiUtils.unstable_useId as jest.Mock;

  beforeEach(async () => {
    di = getDiForUnitTesting();

    let idCounter = 0;
    reactUseIdSpy = jest.spyOn(React, "useId").mockImplementation(() => `namespace-tree-test-id-${++idCounter}`);
    muiUseIdMock.mockImplementation(() => `namespace-tree-test-id-${++idCounter}`);
  ...
  afterEach(() => {
    reactUseIdSpy.mockRestore();
    muiUseIdMock.mockReset();
  });
  ...
  it("renders one namespace without children", () => {
    ...
    const items = Array.from(result.getAllByRole("treeitem"), (node) => node.getAttribute("data-testid"));

    expect(items).toEqual(["namespace-single-root"]);
  });
  ...
  it("renders namespace with 2 children namespaces", () => {
    ...
    expect(items).toEqual(["namespace-acme-org", "namespace-team-a", "namespace-team-b"]);
  });
  ...
  it("renders namespace with children namespaces and a subnamespace", () => {
    ...
    expect(items).toEqual(["namespace-org-a", "namespace-team-c", "namespace-service-1"]);
  });
  ...
  it("renders 2 levels deep", () => {
    ...
    expect(items).toEqual([
      ...
      "namespace-level-deep-child-a",
      "namespace-level-deep-child-b",
      "namespace-level-deep-subchild-a",
    ]);
  });
  ...
  it("collapses item by clicking minus button", () => {
    ...
    expect(levelB).toHaveAttribute("aria-expanded", "false");
  });
  ...
  it("expands item by clicking plus button", () => {
    ...
    expect(levelB).toHaveAttribute("aria-expanded", "true");
  });
});
