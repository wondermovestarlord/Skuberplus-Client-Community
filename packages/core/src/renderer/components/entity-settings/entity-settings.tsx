...
const NonInjectedCatalogEntitySettings = observer((props: CatalogEntitySettingsProps & CatalogEntitySettingsDeps) => {
  const { activeEntitySettingsTab, entity } = props;
      ...
      <Tabs className="flex column" scrollable={false} onChange={activeEntitySettingsTab.set} value={tabId}>
        {groups.map(({ tabs, title }) => (
          <React.Fragment key={title}>
            ...
            {tabs.map((setting) => (
              <Tab key={setting.id} value={setting.id} label={setting.title} data-testid={`${setting.id}-tab`} />
            ))}
          </React.Fragment>
        ))}
      </Tabs>
  ...
  return (
    <SettingLayout navigation={renderNavigation()} contentGaps={false} data-testid="entity-settings">
      {tabId && setting ? (
        <section>
...
const CatalogEntitySettings = withInjectables<CatalogEntitySettingsDeps, CatalogEntitySettingsProps>(
  NonInjectedCatalogEntitySettings,
  {
    getProps: (di, props) => ({
      ...props,
