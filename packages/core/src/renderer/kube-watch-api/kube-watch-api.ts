...
interface Dependencies {
  ...
  readonly logger: Logger;
  readonly appActivityState: AppActivityState;
}
...
export class KubeWatchApi {
  ...
  private subscribeStore({ store, parent, namespaces, onLoadFailure }: SubscribeStoreParams): Disposer {
    ...
    // Tracks whether watches are paused due to app being in background.
    // Checked by the namespace reaction to avoid restarting watches while paused.
    let isPaused = false;

    // Pause watches when the app goes to background, resume on foreground.
    const cancelActivityReaction = reaction(
      () => this.dependencies.appActivityState.isActive,
      (isActive) => {
        if (!isActive) {
          isPaused = true;
          this.dependencies.logger.info(`[KUBE-WATCH-API]: pausing watch for ${store.api.apiBase} (app backgrounded)`);
          childController.abort();
          unsubscribe();
        } else {
          isPaused = false;
          this.dependencies.logger.info(`[KUBE-WATCH-API]: resuming watch for ${store.api.apiBase} (app foregrounded)`);
          childController = new WrappedAbortController(parent);
          const resumeNamespaces = isNamespaceFilterWatch
            ? (this.dependencies.clusterContext.contextNamespaces?.slice() ?? [])
            : namespaces;
          loadThenSubscribe(resumeNamespaces).catch(noop);
        }
      },
    );
      ...
      isNamespaceFilterWatch && store.api.isNamespaced
        ? reaction(
            ...
            ([namespaces, curSelectedAll], [prevNamespaces, prevSelectedAll]) => {
              if (isPaused) {
                return; // Skip namespace changes while app is in background
              }
