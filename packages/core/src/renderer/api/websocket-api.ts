...
export class WebSocketApi<Events extends WebSocketEvents> extends (EventEmitter as {
  ...
  /**
   * 🎯 목적: 소켓이 활성 상태인지 확인 (연결 중 또는 연결됨)
   *
   * 📝 주의사항:
   * - socket 직접 접근 대신 이 메서드 사용 권장
   * - CLOSED가 아니면 활성 상태로 간주
   *
   * @returns 소켓이 활성 상태(CONNECTING, OPEN)면 true
   */
  ...
  protected _onClose(evt: CloseEvent) {
    ...
    if (error) {
      ...
      if (reconnectDelay && this.socket) {
        ...
        // 🎯 버그 수정: reconnectDelay 없어도 CLOSED로 설정
