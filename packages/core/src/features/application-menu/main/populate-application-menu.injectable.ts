...
const toHierarchicalElectronMenuItem = (composite: Composite<ApplicationMenuItemTypes>): MenuItemOpts => {
  ...
  switch (value.kind) {
    ...
    case "os-action-menu-item": {
      const { label, keyboardShortcut, actionName, electronVisible } = value;

      // electronVisible 값 계산 (IComputedValue 또는 boolean)
      const visible =
        electronVisible === undefined
          ? undefined
          : typeof electronVisible === "boolean"
            ? electronVisible
            : electronVisible.get();
