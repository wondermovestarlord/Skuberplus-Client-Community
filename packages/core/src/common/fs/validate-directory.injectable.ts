    ...
    return async (path) => {
      try {
        ...
        return {
          ...
          error: `the provided path is ${getUserReadableFileType(stats)} and not a directory.`,
        };
      } catch (error) {
        ...
        const humanReadableErrors: Record<string, string> = {
          ...
          ELOOP:
            "the provided path is a sym-link which points to a chain of sym-links that is too long to resolve. Perhaps it is cyclic.",
          ENAMETOOLONG: "the pathname is too long to be used.",
          ENOTDIR: "a prefix of the provided path is not a directory.",
        };

        const humanReadableError = error.code ? humanReadableErrors[error.code] : lowerFirst(String(error));
