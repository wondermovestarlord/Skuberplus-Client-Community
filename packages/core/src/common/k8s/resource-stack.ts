...
export class ResourceStack {
  ...
  protected async renderTemplates(folderPath: string, templateContext: any): Promise<string[]> {
    ...
    for (const filename of files) {
      ...
      const data = (filename.endsWith(".hb") ? this.renderHandlebarsTemplate(file, raw, templateContext) : raw).trim();
  ...
  private renderHandlebarsTemplate(filePath: string, source: string, templateContext: any) {
    let template = this.templateCache.get(filePath);

    if (!template) {
      template = precompileHandlebarsTemplate(source);
