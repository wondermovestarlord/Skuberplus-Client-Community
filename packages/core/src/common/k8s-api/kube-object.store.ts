...
> extends ItemStore<K> {
  ...
  protected bindWatchEventsUpdater(delay = 1000) {
    reaction(
      () => {
        // eventsBuffer.length를 명시적으로 읽어서 MobX 추적에 포함
        // 스프레드 연산자만 사용하면 배열 변경 감지가 안 됨
        void this.eventsBuffer.length;
  ...
  private watchNamespace(namespace: string, abortController: AbortController, opts: KubeObjectStoreSubscribeParams) {
    const resourceVersion = this.api.getResourceVersion(namespace);
  ...
  protected updateFromEventsBuffer() {
    ...
    // update items
    const finalItems = this.sortItems(items.slice(-this.bufferSize));
    this.items.replace(finalItems);
  }
}
